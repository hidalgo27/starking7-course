...
{{--            Marcar como culminado--}}
            ...
            <div class="card mt-2">
                <div class="card-body flex text-gray-500 font-bold">
                    ...
                    @if($this->next)
                        <a wire:click="changeLesson({{$this->next}})" class="ml-auto cursor-pointer">Suguiente tema</a>
                    @endif
                </div>
            </div>
                ...
                <ul>
                    @foreach($course->sections as $section)
                        <li class="text-gray-600 mb-4">
                            <button type="button" class="font-bold text-base mb-2">{{$section->name}}</button>
                            <ul>
                                @foreach($section->lessons as $lesson)
                                    <li class="flex">
                                        ...
                                        <button class="text-base text-left" type="button" wire:click="changeLesson({{$lesson}})" >{{$lesson->name}}</button>
                                    </li>
                                @endforeach
                            </ul>
                        </li>
